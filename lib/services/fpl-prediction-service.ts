...
export class FPLPredictionService {
  ...
  /**
   * Generates xPts projections for specified players over the next N gameweeks.
   */
  async getProjections(
    ...
    // 1. Games played per team (finished matches)
    ...
    // 3. Fetch players + teams + opps for target GWs
    ...
    const predictions = players
      .map((player) => {
        ...
        const teamPM = {
          ...
          ppda_season: teamSeason?.ppda ?? 12.0,
        };
        ...
        const gwData: Record<number, { xPts: number; fixture: string; opponent: string; isHome?: boolean; breakdown?: any; raw?: any }> = {};
        let totalXPts = 0;
