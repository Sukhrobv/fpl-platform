...
function readSeasonCode(): string {
  ...
  if (!season || !/^20\d{2}\/\d{2}$/.test(season)) {
    throw new Error("Usage: tsx scripts/sync-understat.ts --season=2025/26");
  }
  return season;
}
...
async function syncUnderstat() {
  ...
  try {
    const seasonCode = readSeasonCode();
    const season = await prisma.season.findUnique({
      where: { code: seasonCode },
      select: { id: true },
    });
    if (!season) throw new Error(`Season ${seasonCode} is not configured`);
    const understatYear = Number(seasonCode.slice(0, 4));
    // 1. Fetch Data
    const players = await collector.getLeaguePlayers("EPL", understatYear);
    console.log(`📥 Fetched ${players.length} players from Understat`);

    // 2. Get FPL Players for mapping
    const registrations = await prisma.seasonPlayer.findMany({
      ...
      select: {
        ...
        seasonTeam: { select: { name: true, shortName: true } },
      },
    });
    const seasonPlayerByPlayerId = new Map(
    ...
    const candidates = registrations.map((registration) => ({
      id: registration.player.id.toString(),
      name: `${registration.player.firstName} ${registration.player.secondName}`, // Full name usually works best
      webName: registration.player.webName,
      team: registration.seasonTeam.name,
    }));
    ...
    for (const uPlayer of players) {
        ...
        if (match && match.confidence > 0.7) {
          ...
          if (existingMapping) {
            ...
            if (existingMapping.externalId !== uPlayer.id) {
              console.log(
                `🔄 Updating mapping for ${uPlayer.player_name} (ID: ${existingMapping.externalId} -> ${uPlayer.id})`,
              );
            ...
            await prisma.playerMapping.create({
              data: {
                ...
                method:
                  match.method === "EXACT" ? "EXACT_MATCH" : "FUZZY_MATCH",
      ...
      if (fplPlayerId) {
        const seasonPlayer = seasonPlayerByPlayerId.get(fplPlayerId);
        if (!seasonPlayer) {
          console.warn(
            `⚠️ Player ${fplPlayerId} is not registered in ${seasonCode}`,
          );
          continue;
        }
        ...
        await prisma.externalPlayerStats.upsert({
          where: {
            seasonPlayerId_gameweek_source: {
    ...
    const teamsData = await collector.getLeagueTeams("EPL", understatYear);
    ...
    for (const [understatId, teamData] of Object.entries(teamsData)) {
      ...
      const seasonTeam = await prisma.seasonTeam.findFirst({
        where: { seasonId: season.id, name: fplName },
        include: { team: true },
      });

      if (!seasonTeam) {
        console.warn(
          `⚠️ Could not find FPL team for Understat team: ${understatName} (Mapped to: ${fplName})`,
        );
        continue;
      }

      // Update Team mapping if needed
      if (!seasonTeam.team.understatId) {
        await prisma.team.update({
          where: { id: seasonTeam.team.id },
